            ...
            @foreach (config('perusahaan.rencana') as $item)
                @php
                    ...
                    $gaya = match ($item['status']) {
                        'selesai' => ['Selesai', 'bg-brand-600', 'bg-slate-50 text-brand-700'],
                        'berjalan' => ['Sedang berjalan', 'bg-brand-600', 'bg-slate-50 text-brand-700'],
                        default => ['Rencana', 'bg-slate-300', 'bg-slate-50 text-slate-600'],
                    };
                @endphp
